...
class TextFunctions
{
	public static function implement(funk:FunkinLua)
	{
		...
		Lua_helper.add_callback(lua, "makeLuaWiiText", function(tag:String, text:String, width:Int, x:Float, y:Float) {
			tag = tag.replace('.', '');
			LuaUtils.resetSpriteTag(tag);
			var leText = new WiiCustomText(x, y, width, text, 1);
			leText.cameras = [game.camHUD];
			leText.scrollFactor.set();
			game.variables.set(tag, leText);
		});
