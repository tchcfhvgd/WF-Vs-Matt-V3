...
class TitleState extends MusicBeatState
{
	...
	override public function create():Void
	{
		...
		pointerGraphic.persist = true;
		Paths.dumpExclusions.push("assets/shared/images/pointer.png");
		FlxG.mouse.load(pointerGraphic.bitmap,1,-22,-6);
		...
		if(!HealthNSafety.leftState) {
			if (FlxG.save.data.corrupted != null && FlxG.save.data.corrupted) {
				FlxTransitionableState.skipNextTransIn = true;
				FlxTransitionableState.skipNextTransOut = true;
				MusicBeatState.switchState(new CorruptedState());
			} else {
				FlxTransitionableState.skipNextTransIn = true;
				FlxTransitionableState.skipNextTransOut = true;
				MusicBeatState.switchState(new HealthNSafety());
			}
		} else {
			if (initialized)
				startIntro();
