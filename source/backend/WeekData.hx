...
class WeekData {
	...
	public static function reloadWeekFiles(isStoryMode:Null<Bool> = false)
	{
		...
		var week:WeekFile = {
			songs: [["Funkin Corner","bf",[244,58,118]]],
			weekCharacters: [],
			weekBackground: "",
			weekBefore: "tutorial",
			storyName: "Wii Funkin' Stupid Alts",
			weekName: "Stupid Alts",
			freeplayColor: [0,0,0],
			startUnlocked: true,
			hiddenUntilUnlocked: false,
			difficulties: "Hard",
		...
		var data = new WeekData(week, "corner");
		if((PlayState.isStoryMode && !data.hideStoryMode) || (!PlayState.isStoryMode && !data.hideFreeplay))
		{
			weeksLoaded.set("corner", data);
			weeksList.push("corner");
		}
	}
